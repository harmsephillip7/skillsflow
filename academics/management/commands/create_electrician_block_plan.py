"""
Create Block Plan Template for QCTO Occupational Certificate: Electrician (SAQA ID 91761)
Creates a 3-year full-time apprenticeship structure with alternating institutional and workplace phases.
"""

from django.core.management.base import BaseCommand
from django.utils import timezone
from academics.models import Qualification, Module, BlockPlanTemplate, BlockPlanPhase, BlockPlanModuleSlot
from datetime import date


class Command(BaseCommand):
    help = 'Create Block Plan Template for QCTO Electrician qualification'

    def handle(self, *args, **options):
        self.stdout.write('Creating Electrician Block Plan Template...')
        ...
        # Create the Block Plan Template
        template, created = BlockPlanTemplate.objects.update_or_create(
            qualification=qualification,
            name='3-Year Full-Time Apprenticeship',
            defaults={
                'description': '''Standard 3-year apprenticeship block plan for the Occupational Certificate: Electrician.
...
Integration/Trade Test Prep: 32 weeks''',
        ...
        for phase_data in phases_data:
            phase = BlockPlanPhase.objects.create(
                template=template,
            ...
            if phase_data['modules']:
                for seq, (module_code, credits) in enumerate(phase_data['modules'], start=1):
                    try:
                        ...
                        slot = BlockPlanModuleSlot.objects.create(
                            phase=phase,
                            module=module,
                            sequence=seq,
        ...
        self.stdout.write(self.style.SUCCESS('Block Plan Creation Summary'))
        self.stdout.write(self.style.SUCCESS('=' * 60))
        self.stdout.write(f'Template: {template.name}')
        self.stdout.write(f'Qualification: {qualification.title}')
        self.stdout.write(f'Total Duration: {template.total_weeks} weeks (3 years)')
        self.stdout.write('')
        ...
        self.stdout.write('')
        self.stdout.write(self.style.SUCCESS('Block plan created successfully!'))
